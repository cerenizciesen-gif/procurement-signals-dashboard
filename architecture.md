...
| Model access | API key in the Lambda environment | `DEEPSEEK_API_KEY` as a repository secret |
...
The generative layer is active and uses `deepseek-v4-flash`.

External news headlines are classified by the model into the predefined risk taxonomy.
After the risk factor is identified, the affected materials are derived deterministically
from the taxonomy. This prevents unsupported material-risk combinations.

Negotiation arguments are also generated by `deepseek-v4-flash`. The model receives
already computed prices, delay costs, risk information and the Buy / Delay decision as
fixed inputs. It is not responsible for calculating monetary values or making the final
procurement decision.

The workflow uses the repository secret `DEEPSEEK_API_KEY`. If the model or API key is
temporarily unavailable, the scripts can continue with their predefined fallback logic
instead of blocking the entire signal-generation process.
...
| Public news search feed | Headlines for freight, wage, supply and sanction topics | Classified by `deepseek-v4-flash` into the predefined risk taxonomy; keyword rules remain available as fallback |
