...
The DeepSeek integration is active.

External news headlines are classified by `deepseek-v4-flash`. The model identifies
whether a headline is relevant and assigns it to one of the predefined risk factors.

The affected materials are then derived deterministically from the predefined
risk taxonomy. This prevents unsupported material-risk combinations.

The negotiation arguments are also generated by `deepseek-v4-flash`. However,
the model does not calculate prices, delay costs, or the Buy / Delay decision.
Those values are produced by deterministic business logic and are passed to the
model as fixed inputs.

If the DeepSeek API is temporarily unavailable, the scripts can continue using
their fallback logic instead of stopping the complete workflow.
...
| --- | --- | --- | --- |
| `DEEPSEEK_API_KEY` | Repository secret | configured | Activates DeepSeek-based news classification and negotiation-argument generation |
| `USE_LIVE_MACRO` | Environment variable | `"0"` | `"1"` lets detected live risks drive the recommendation instead of the dataset |

To configure the model, add a repository secret under
**Settings → Secrets and variables → Actions → New repository secret**
with the name `DEEPSEEK_API_KEY`.
...
The workflow passes this secret to the Python scripts at runtime. If the secret
is missing or the model request fails, the application can use its defined
fallback logic.
...
| News screening | Real feed; classified by `deepseek-v4-flash` with fallback logic if the model is unavailable |
