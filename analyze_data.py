...
def generate_argument(
    ...
    """Generate a negotiation argument, retrying transient empty responses."""
    ...
    def fall_back(note: str) -> tuple[str, str]:
        if kept:
            print(
                f"  {material}: {note}, "
                "keeping the argument from the previous run"
            )
    ...
    for attempt in range(1, 4):
        ...
        try:
            ...
            if text:
                print(
                    f"  {material}: argument generated by "
                    f"{ANTHROPIC_MODEL} on attempt {attempt}"
                )
        ...
        print(
            f"  {material}: attempt {attempt} failed "
            f"({last_error})"
        )
